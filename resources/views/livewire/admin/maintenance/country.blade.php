<div>
    <x-container title="Countries Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true">
            ...
            <x-card title="">
                ...
                <x-table.table loading="true" loadingtarget="paginated">
                    <x-slot name="thead">
                        ...
                        <x-table.table-header class="text-left" value="ABBR" sort="" />
                    ...
                    <x-slot name="tbody">
                        @foreach ($data as $key => $country)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    <x-button
                                        wire:click="openUpdateModal({{ $country->id }})"
                                        xs
                                        icon="pencil-alt"
                                        orange
                                        label="Edit"
                                    />
                                    <x-button
                                        wire:click="delete({{ $country->id }},'{{ $country->description }}')"
                                        xs
                                        icon="trash"
                                        red
                                        label="Delete"
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        <div class="grid gap-4 my-2 lg:grid-cols-2">
            <div class="tooltip buttom" title="ABBR must be alphabetic and maximum 2 characters">
                <x-input wire:model="abbr" label="ABBR" placeholder="" class="uppercase"/>
            </div>
            <div class="tooltip buttom" title="Country must be alphabetic">
