<div>
    <x-container title="Employer List Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true" >
                    ...
                    <x-slot name="tbody">
                        @foreach ($data as $key => $employerList)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    {{ $employerList->employer_id }}
                                </x-table.table-body>

                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    {{ $employerList->employer_name }}
                                </x-table.table-body>
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        ...
        <div class="grid gap-4 my-2 lg:grid-cols-2 ">
            <div class="tooltip buttom" title="Input should consist of letters only (e.g., 'BMMB')">
                <x-input wire:model="employer_id" label="Code " placeholder="" class="uppercase " />
            </div>
            <div class="tooltip buttom" title="Enter the employer list description (e.g., 'CIMB BANK BERHAD')">
                <x-input wire:model="employer_name" label="{{ $modalDescription }}" placeholder="" class="uppercase "/>
            </div>
        </div>
