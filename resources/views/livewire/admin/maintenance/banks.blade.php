<div>
            ...
            <x-card title="">
                <div class="flex items-center justify-between w-full mb-4">
                    ...
                    <div  class="flex items-center space-x-2">
                        ...
                        <select  wire:model.live.debounce.1500ms ="orderBy" 
                            ...
                            <option value="bank_cust">Bank Customer</option>
                        </select>
                ...
                <x-table.table loading="true" loadingtarget="paginated,searchQuery, orderBy">
                    <x-slot name="thead">
                        ...
                        <x-table.table-header class="text-left" value="USER DESCRIPTION" sort="" />
                        ...
                        <x-table.table-header class="text-left" value="CLIENT BANK" sort="" />
                        <x-table.table-header class="text-left" value="CUSTOMER BANK" sort="" />
                            ...
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    @if($bank->bank_client == 'Y')
                                        <x-badge flat emerald label="YES" />
                                    @else
                                        <x-badge flat negative  label="NO" />
                                    @endif
                                </x-table.table-body>
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        <div class="grid gap-4 my-2 lg:grid-cols-2 ">
            <div class="tooltip buttom" title="Input shoulds be a number, limited to 2 digits">
                <x-input wire:model="code" label="Code " placeholder="" class="uppercase " />
            </div>

            <div class="tooltip buttom" title="">
                <x-input wire:model="sys_desc" label="User Description" placeholder="" class="uppercase " disabled/>
            </div>
...
{{-- 
        ...
        <div  class="flex space-x-2 items-center my-6 ">
            <x-toggle md wire:model.defer="status" left-label="Status" />
            <x-toggle md wire:model.defer="bank_client" left-label="Client Bank" />
            <x-toggle md wire:model.defer="bank_cust" left-label="Customer Bank" />
        </div>
        <x-slot name="footer">
