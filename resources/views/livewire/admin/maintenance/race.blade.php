<div>
    <x-container title="Race Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true" >
                    ...
                    <x-slot name="tbody">
                        ...
                        @foreach ($data as $key => $race)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    <x-button
                                        wire:click="openUpdateModal({{ $race->id }})"
                                        xs
                                        icon="pencil-alt"
                                        orange
                                        label="Edit"
                                    />
                                    <x-button
                                        wire:click="delete({{ $race->id }},'{{ $race->description }}')"
                                        xs
                                        icon="trash"
                                        red
                                        label="Delete"
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        <div class="grid gap-4 my-2 lg:grid-cols-2">
            ...
            <div class="tooltip buttom" title="race must be alphabetic">
