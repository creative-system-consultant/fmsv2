<div>
    <x-container title="Inst Mode Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true" >
                    ...
                    <x-slot name="tbody">
                        @foreach ($data as $key => $instMode)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    {{ $instMode->description }}
                                </x-table.table-body>

                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    <x-button 
                                        wire:click="openUpdateModal({{ $instMode->id }})"
                                        sm  
                                        icon="pencil-alt" 
                                        orange 
                                        label="Edit" 
                                    />
                                    <x-button 
                                        wire:click="delete('{{ $instMode->id }}' , '{{ $instMode->code }}')"
                                        sm  
                                        icon="trash" 
                                        red 
                                        label="Delete" 
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        ...
        <div class="grid gap-4 my-2 lg:grid-cols-2 ">
            ...
            <div class="tooltip buttom" title="Description should represent the Inst Mode (e.g., 'ONLINE BANKING')">
