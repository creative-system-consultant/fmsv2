<div>
    <x-container title="Guarantor Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true" >
        <div class="grid grid-cols-1">
            <x-card title="">
                ...
                <x-table.table loading="true" loadingtarget="paginated,searchQuery">
                    ...
                    <x-slot name="tbody">
                        @foreach ($data as $key => $guarantorStatus)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    {{ $guarantorStatus->code }}
                                </x-table.table-body>

                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    {{ $guarantorStatus->description }}
                                </x-table.table-body>

                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    <x-button 
                                        wire:click="openUpdateModal({{ $guarantorStatus->id }})"
                                        sm  
                                        icon="pencil-alt" 
                                        orange 
                                        label="Edit" 
                                    />
                                    <x-button 
                                        wire:click="delete('{{ $guarantorStatus->id }}' , '{{ $guarantorStatus->code }}' , '{{ $guarantorStatus->description }}')"
                                        sm  
                                        icon="trash" 
                                        red 
                                        label="Delete" 
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        ...
        <div class="grid gap-4 my-2 lg:grid-cols-2 ">
            ...
            <div class="tooltip buttom" title="Description of name should represent the guarantor status (e.g., 'ACTIVE')">
