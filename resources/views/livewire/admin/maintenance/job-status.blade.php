<div>
    <x-container title="Job Status Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true" >
                    ...
                    <x-slot name="tbody">
                        @foreach ($data as $key => $jobstatus)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    <x-button 
                                        wire:click="openUpdateModal({{ $jobstatus->id }})"
                                        sm  
                                        icon="pencil-alt" 
                                        orange 
                                        label="Edit" 
                                    />
                                    <x-button 
                                        wire:click="delete({{ $jobstatus->id }}, '{{ $jobstatus->code }}' , '{{ $jobstatus->description }}')"
                                        sm  
                                        icon="trash" 
                                        red 
                                        label="Delete" 
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        ...
        <div class="grid gap-4 my-2 lg:grid-cols-2 ">
            <div class="tooltip buttom" title="Input should consist of one letters only">
                <x-input wire:model="code" label="Code " placeholder="" class="uppercase " />
            </div>
            <div class="tooltip buttom" title="Enter the job status description(e.g., 'EMPLOYED')">
