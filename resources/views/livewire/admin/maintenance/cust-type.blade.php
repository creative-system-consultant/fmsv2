<div>
    <x-container title="Customer Type Maintenance" routeBackBtn="{{route('setting.setting-list')}}" titleBackBtn="setting list" disableBackBtn="true" >
            ...
            <x-card title="">
                ...
                <x-table.table loading="true" loadingtarget="paginated,searchQuery">
                    <x-slot name="thead">
                        ...
                        <x-table.table-header class="text-left" value="CUSTOMER STATUS" sort="" />
                        <x-table.table-header class="text-left" value="DESCRIPTION" sort="" />
                        <x-table.table-header class="text-left" value="ACTION" sort="" />
                    </x-slot>
                    <x-slot name="tbody">
                        @foreach ($data as $key => $custType)
                            <tr>
                                ...
                                <x-table.table-body colspan="" class="text-xs font-medium text-gray-700 ">
                                    <x-button 
                                        wire:click="openUpdateModal('{{ $custType->id }}')"
                                        sm  
                                        icon="pencil-alt" 
                                        orange 
                                        label="Edit" 
                                    />
                                    <x-button 
                                        wire:click="delete('{{ $custType->id }}' , '{{ $custType->cust_type }}' , '{{ $custType->description }}')"
                                        sm  
                                        icon="trash" 
                                        red 
                                        label="Delete" 
    ...
    <x-modal.card title="{{ $modalTitle }}" align="center" blur wire:model.defer="openModal" max-width="lg">
        ...
        <div class="grid gap-4 my-2 lg:grid-cols-2 ">
            <div class="tooltip buttom" title="Input should consist of one letters only(eg.'A')">
                <x-input wire:model="cust_type" label="Customer Type " placeholder="" class="uppercase " />
            </div>
            <div class="tooltip buttom" title="Description of name should represent the customer type (e.g.,'BIMBB STAFF')">
